/* Live startup trace panel for Dispatch Simulator loading screens — v1.6.15. */
(() => {
  'use strict';
  ...
  function describe(game, doc) {
    ...
    if (typeof safe(() => game.initializeSimulator, undefined) !== 'function') {
      return {key:'initializeSimulator-missing', text:'WAITING: initializeSimulator() is not defined inside the iframe.'};
    }
    ...
    if (expected && runtimeVersion !== expected) {
      return {key:`runtime-version:${expected}:${runtimeVersion}`, text:`WAITING: city runtime v${expected}; current=${runtimeVersion || 'not started'}.`};
    }
    ...
    if (stage?.stage && stage.stage !== 'ready') {
      return {key:`stage:${stage.source || 'inner'}:${stage.stage}:${stage.detail || ''}`, text:`RUNNING: ${stage.source || 'inner'} → ${stage.stage}${stage.detail ? ` — ${stage.detail}` : ''}.`};
    }
  ...
  window.PTBO_STARTUP_TRACE = Object.freeze({
    version:VERSION,
    entries,
    mark,
    ok,
    ...
    fail,
    setCurrent,
    snapshot:() => ({entries:[...entries], current:currentNode.textContent, frame:frame.src}),
  });
})();
