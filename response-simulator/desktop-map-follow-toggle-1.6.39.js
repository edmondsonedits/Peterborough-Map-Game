/* Desktop map follow toggle — v1.6.39. */
(() => {
  'use strict';
  ...
  function keepDrivingKeysOwnedBySimulator() {
    try {
      // The simulator is intentionally created with Leaflet keyboard:false.
      // Re-enabling Leaflet keyboard makes ArrowLeft/ArrowRight pan the map after
      // the player has clicked/dragged it, which steals steering from the truck.
      mapInstance?.keyboard?.disable?.();
      const container = mapInstance?.getContainer?.();
      if (container && document.activeElement === container) container.blur?.();
    } catch (_) {}
  }

  function patchMapFollow() {
    if (setViewPatched) {
      keepDrivingKeysOwnedBySimulator();
      return true;
    }
    try {
  ...
  function recenterAndFollow() {
    ...
    requestAnimationFrame(keepDrivingKeysOwnedBySimulator);
  }
