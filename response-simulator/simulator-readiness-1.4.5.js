...
(() => {
  ...
  function installVersionBadge() {
    ...
    style.textContent = '#ptbo-version-badge{margin-top:18px!important;color:#9ca3af!important;font-size:8px!important;font-weight:700!important;letter-spacing:.08em!important;text-align:right!important;opacity:.58!important}#ptbo-version-badge::after{content:""!important}';
    return true;
  }
  ...
  async function initialize() {
    ...
    const roadsReady = window.PTBO_ROAD_COLLISION_BOOTSTRAP_READY || (async () => {
    ...
    if (isMobileWrapper) {
      await waitForValue(() => instruments.state?.mobileSteeringConnected,'Mobile steering connection',10000);
      await waitForValue(() => window.PTBO_STABLE_MOBILE_CAMERA?.state?.installed,'Stable mobile camera',10000);
    }
