/* Live startup trace panel for Dispatch Simulator loading screens — v1.6.17. */
(() => {
  'use strict';
  ...
  function describe(game, doc) {
    ...
    const readinessError = safe(() => game.PTBO_SIMULATOR_READY_ERROR, null);
    ...
    if (readinessError) return {key:`readiness-error:${stringifyDetail(readinessError)}`,text:`ERROR: simulator readiness failed — ${stringifyDetail(readinessError)}`,level:'error'};
    ...
    if (!safe(() => game.PTBO_SIMULATOR_READY, null)) {
      const readinessVersion = safe(() => game.PTBO_SIMULATOR_READY_VERSION, null);
      if (readinessVersion) return {key:`simulator-readiness-created:${readinessVersion}`,text:`WAITING: simulator readiness v${readinessVersion} created its gate.`};
      ...
      return {key:'simulator-readiness-script',text:'WAITING: simulator-readiness script has not created its startup gate.'};
    }
