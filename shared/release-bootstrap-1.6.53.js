/* v1.6.53 production release bootstrap: cache-bust + desktop incident tablet delivery + dispatch minimizer. */
(() => {
  'use strict';
  ...
  function installDesktopTabletBridge() {
    ...
    const inject = () => {
      ...
      if (game.PTBO_RESPONSE_TABLET?.version !== '1.6.48') {
        ensureScript(
          doc,
          'ptbo-release-tablet-v1653',
          new URL('../response-tablet-1.6.48.js?v=1.6.53', location.href).href
        );
      }

      if (game.PTBO_TABLET_CLOSE_DISPATCH?.version !== VERSION) {
        ensureScript(
          doc,
          'ptbo-tablet-close-dispatch-v1653',
          new URL('../tablet-close-dispatch-1.6.53.js?v=1.6.53', location.href).href
        );
      }

      return game.PTBO_RESPONSE_TABLET?.version === '1.6.48' && game.PTBO_TABLET_CLOSE_DISPATCH?.version === VERSION;
    };
