/* v1.6.54 production release bootstrap: desktop tablet delivery, dispatch minimizer, and stable tablet button label. */
(() => {
  'use strict';
  ...
  function installDesktopTabletBridge() {
    ...
    const inject = () => {
      ...
      if (game.PTBO_RESPONSE_TABLET?.version !== '1.6.48') {
        ensureScript(
          doc,
          'ptbo-release-tablet-v1654',
          new URL('../response-tablet-1.6.48.js?v=1.6.54', location.href).href
        );
      }
      ...
      if (game.PTBO_TABLET_CLOSE_DISPATCH?.version !== '1.6.53') {
        ensureScript(
          doc,
          'ptbo-tablet-close-dispatch-v1654',
          new URL('../tablet-close-dispatch-1.6.53.js?v=1.6.54', location.href).href
        );
      }

      if (game.PTBO_TABLET_BUTTON_STABILITY?.version !== VERSION) {
        ensureScript(
          doc,
          'ptbo-tablet-button-stability-v1654',
          new URL('../tablet-button-stability-1.6.54.js?v=1.6.54', location.href).href
        );
      }

      return game.PTBO_RESPONSE_TABLET?.version === '1.6.48'
        && game.PTBO_TABLET_CLOSE_DISPATCH?.version === '1.6.53'
        && game.PTBO_TABLET_BUTTON_STABILITY?.version === VERSION;
    };
