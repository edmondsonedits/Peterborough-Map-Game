...
(() => {
  ...
  function injectIntoFrame(doc,id,relativeUrl){const existing=doc.getElementById(id);if(existing)return existing;const script=doc.createElement('script');script.id=id;script.src=new URL(relativeUrl,SCRIPT_URL).href;(doc.body||doc.documentElement).appendChild(script);return script}

  function injectPageScript(id,relativeUrl){return new Promise((resolve,reject)=>{const existing=document.getElementById(id);if(existing){if(existing.dataset.ptboLoaded==='true')return resolve(existing);existing.addEventListener('load',()=>resolve(existing),{once:true});existing.addEventListener('error',()=>reject(new Error(`Unable to load ${relativeUrl}.`)),{once:true});return}const script=document.createElement('script');script.id=id;script.src=new URL(relativeUrl,SCRIPT_URL).href;script.onload=()=>{script.dataset.ptboLoaded='true';resolve(script)};script.onerror=()=>reject(new Error(`Unable to load ${relativeUrl}.`));document.head.appendChild(script)})}

  function selectedCityId(){const params=new URLSearchParams(location.search);let stored=null;try{stored=localStorage.getItem('ptboSelectedCity')}catch(_){}const requested=String(params.get('city')||stored||'peterborough').toLowerCase();return /^[a-z0-9-]+$/.test(requested)?requested:'peterborough'}

  function installCitySelector(){if(!document.getElementById('dispatch-game-link'))return;injectPageScript('ptbo-city-registry-loader',`../cities/city-registry.js?v=${VERSION}`).then(()=>injectPageScript('ptbo-city-selector-loader',`city-selector.js?v=${VERSION}`)).catch(error=>console.error('City selector failed to initialize.',error))}

  function normalizeSimulatorFrameUrl(frame){
    if(!frame || frame.dataset.ptboCityUrlVersion===VERSION)return;
  ...
  function installResponseEnhancements(){
    ...
    const finishSatelliteCover=()=>{if(completed)return;completed=true;if(!cover)return;cover.classList.add('hidden');setTimeout(()=>{cover?.remove();coverStyle?.remove()},240)};
    const installCover=()=>{if(document.getElementById('ptbo-satellite-startup-cover'))return;coverStyle=document.createElement('style');coverStyle.id='ptbo-satellite-startup-style';coverStyle.textContent=`#ptbo-satellite-startup-cover{position:fixed;inset:0;z-index:5950;display:grid;place-items:center;padding:24px;color:#f8fafc;background:#111827;text-align:center;transition:opacity .2s ease;font-family:Inter,ui-sans-serif,system-ui,-apple-system,BlinkMacSystemFont,"Segoe UI",sans-serif}#ptbo-satellite-startup-cover.hidden{opacity:0;pointer-events:none}#ptbo-satellite-startup-cover strong{display:block;font-size:1.15rem}#ptbo-satellite-startup-cover span{display:block;margin-top:7px;color:#cbd5e1;font-size:.9rem;line-height:1.45}`;cover=document.createElement('div');cover.id='ptbo-satellite-startup-cover';cover.innerHTML='<div><strong>Loading satellite map</strong><span>Preparing imagery, city package, camera, and driving controls…</span></div>';document.head.appendChild(coverStyle);document.body.appendChild(cover)};
    installCover();
    ...
    const installInsideFrame=()=>{
      ...
      const finishWhenReady=()=>{Promise.resolve(game.PTBO_SATELLITE_MAP_READY).then(finishSatelliteCover,finishSatelliteCover)};
      if(game.PTBO_SATELLITE_MAP_READY)finishWhenReady();else{satellite.addEventListener('load',finishWhenReady,{once:true});satellite.addEventListener('error',finishSatelliteCover,{once:true})}
    };
  ...
  function installPageEnhancements(){const isMobile=/\/response-simulator\/mobile\/(?:index\.html)?$/.test(location.pathname);if(isMobile&&!document.getElementById('ptbo-mobile-dispatch-hud-loader')){const script=document.createElement('script');script.id='ptbo-mobile-dispatch-hud-loader';script.src=new URL(`mobile-dispatch-hud-1.5.5.js?v=${VERSION}`,SCRIPT_URL).href;script.async=true;document.head.appendChild(script)}installCitySelector();installResponseEnhancements()}
