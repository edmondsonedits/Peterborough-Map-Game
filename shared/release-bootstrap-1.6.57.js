/* v1.6.64 production release bootstrap: resilient street-map tablet and corrected launcher cache wiring. */
(() => {
  'use strict';
  ...
  function installDesktopTabletBridge() {
    ...
    const inject = () => {
      ...
      if (game.PTBO_RESPONSE_TABLET?.version !== VERSION) {
        ensureScript(doc, 'ptbo-release-tablet-v1664', new URL('../response-tablet-1.6.48.js?v=1.6.64&map=street', location.href).href);
      }
      if (game.PTBO_TABLET_CLOSE_DISPATCH?.version !== '1.6.53') {
        ensureScript(doc, 'ptbo-tablet-close-dispatch-v1664', new URL('../tablet-close-dispatch-1.6.53.js?v=1.6.64', location.href).href);
      }
      if (game.PTBO_TABLET_BUTTON_STABILITY?.version !== '1.6.54') {
        ensureScript(doc, 'ptbo-tablet-button-stability-v1664', new URL('../tablet-button-stability-1.6.54.js?v=1.6.64', location.href).href);
      }
      if (game.PTBO_TRAINING_UI?.version !== '1.6.57') {
        ensureScript(doc, 'ptbo-training-ui-v1664', new URL('../training-ui-1.6.57.js?v=1.6.64', location.href).href);
      }
