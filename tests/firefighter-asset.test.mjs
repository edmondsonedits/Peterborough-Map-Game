...
test('firefighter is self-contained, correctly sized and within geometry budget',async()=>{
 ...
 scene.traverse(n=>{if(n.isMesh){triangles+=(n.geometry.index?.count??n.geometry.attributes.position.count)/3;draws++;assert.equal(n.material.map,null);}});
 assert.ok(triangles<55000);assert.ok(draws<=45);assert.ok(bytes.length<3000000);assert.ok(bounds.min.y>=-.001&&bounds.max.y<=2.1);
 assert.ok(Math.abs(model.leftArm.position.y-1.49)<1e-6);assert.ok(Math.abs(model.leftLeg.position.y-.89)<1e-6);
 const json=JSON.parse(bytes.subarray(20,20+bytes.readUInt32LE(12)).toString());assert.equal(json.images,undefined);
});
