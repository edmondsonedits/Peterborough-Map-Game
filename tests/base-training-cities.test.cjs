...
test('every base-training city ships a v1.6.9 package and deliberately empty dispatch descriptor', () => {
  for(const id of cities){const packageSource=read(`cities/${id}/package.js`),dispatchSource=read(`cities/${id}/dispatch-data.js`);assert.match(packageSource,/const VERSION='1\.6\.9'/,id);assert.match(packageSource,/PTBO_PREVIEW_CITY_FACTORY/,id);assert.match(dispatchSource,/available:false/,id)}
});
...
test('base store refreshes when an asynchronous city package fills its base arrays', () => {
  ...
  const c=browserContext({PTBO_CITY_PACKAGE:{id:'test',name:'Test City',features:{baseTraining:true},roads:{center:[44,-79]},map:{defaultCenter:[44,-79]}},PTBO_SERVICE_CONFIG:{profiles:{fire:{id:'fire',bases:fire},ems:{id:'ems',bases:ems}},hospital,alarmCategories:[]},PTBO_LOCATION_CHANGES:{diff:(before,after)=>({schema:1,updated:[],added:after.filter(item=>!before.some(seed=>seed.id===item.id)),deleted:[]})}});
...
test('v1.6.9 wrapper bootstrap repairs stale inner simulator modules before readiness', () => {
  const build=read('shared/build-version.js'),runtime=read('response-simulator/city-runtime-bootstrap-1.6.9.js'),service=read('response-simulator/service-selection.js');
  assert.match(build,/city-runtime-bootstrap-1\.6\.9\.js/);assert.match(build,/PTBO_CITY_RUNTIME_BOOTSTRAP_EXPECTED_VERSION/);assert.match(runtime,/PTBO_CITY_RUNTIME_READY_VERSION/);assert.match(runtime,/shared\/base-locations\.js/);assert.match(runtime,/response-simulator\/service-mode\.js/);assert.match(service,/runtimeReady\(game\)/);assert.match(service,/PTBO_CITY_RUNTIME_BOOTSTRAP_EXPECTED_VERSION/);
});

test('desktop and mobile construct the selected city iframe before shared startup and show real diagnostics', () => {
  for(const file of ['response-simulator/play/index.html','response-simulator/mobile/index.html']){const source=read(file);assert.match(source,/1\.6\.9/,file);assert.match(source,/url\.searchParams\.set\('city',city\)/,file);assert.match(source,/url\.searchParams\.set\('fresh'/,file);assert.match(source,/shared\/build-version\.js\?v=1\.6\.9/,file);assert.match(source,/simulator-readiness-1\.4\.5\.js/,file);assert.match(source,/diagnostic/,file)}
});

test('city selector launches with a fresh URL so Samsung/mobile caches cannot replay an old wrapper', () => {
  const source=read('shared/city-selector.js');assert.match(source,/const VERSION = '1\.6\.9'/);assert.match(source,/url\.searchParams\.set\('fresh', String\(Date\.now\(\)\)\)/);
});

test('production build marker, base store and launch screen all use v1.6.9', () => {
  assert.match(read('shared/build-version.js'),/const VERSION = '1\.6\.9'/);assert.match(read('shared/base-locations.js'),/const VERSION = '1\.6\.9'/);assert.match(read('index.html'),/shared\/build-version\.js\?v=1\.6\.9/);assert.match(read('cities/preview-package-factory.js'),/const VERSION = '1\.6\.9'/);assert.match(read('cities/peterborough/package.js'),/const VERSION = '1\.6\.9'/);
});
