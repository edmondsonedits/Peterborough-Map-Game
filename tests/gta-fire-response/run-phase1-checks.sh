...
node --test tests/gta-fire-response/phase1-core.test.mjs
python -m http.server 4173 >/tmp/pfr-phase1-http.log 2>&1 &
server_pid=$!
trap 'kill "$server_pid" 2>/dev/null || true' EXIT
for attempt in {1..20}; do
  if curl -fsS 'http://127.0.0.1:4173/gta-fire-response/?test=1' | grep -q 'Peterborough Fire Response'; then break; fi
