...
python -m http.server 4173 >/tmp/pfr-station-yard-http.log 2>&1 &
server_pid=$!
trap 'kill "$server_pid" 2>/dev/null || true' EXIT
for attempt in {1..20}; do
  if curl -fsS 'http://127.0.0.1:4173/gta-fire-response/?test=1&unlock=all&call=apartment-fire-bethune' | grep -q '1.1.1-station-yard'; then break; fi
