/* Dispatch editor base-area + vehicle spawn editor — v1.6.55.
   Fixes the base-selection freeze by avoiding a self-triggering MutationObserver loop. */
(() => {
  'use strict';
  ...
  function installFields() {
    ...
    if (rotationLabel) setTextIfChanged(rotationLabel, 'Area rotation (°)');
  ...
  function renderMarker(ensureValid = true) {
    ...
    if (!spawnMarker) {
      ...
      spawnMarker._ptboHeading = heading;
      spawnMarker._ptboService = service;
      ...
      const current = spawnMarker.getLatLng();
      if (!current || Math.abs(current.lat-position[0]) > 1e-10 || Math.abs(current.lng-position[1]) > 1e-10) spawnMarker.setLatLng(position);
      if (spawnMarker._ptboHeading !== heading || spawnMarker._ptboService !== service) {
  ...
  function polishEditorText() {
    if (!isBaseEditorOpen()) return;
    setTextIfChanged($('base-help'), 'The amber box is the drivable station area. Drag the box to move it, drag its handles to resize width/length, and use Area rotation to turn it.');
    setTextIfChanged($('review-count'), 'Box = drivable area');
    const byId = new Map(bases.getAll().map(base => [base.id, base]));
    for (const button of document.querySelectorAll('#call-list [data-id]')) {
      const base = byId.get(button.dataset.id);
      if (!base) continue;
      ...
      setTextIfChanged(spans[spans.length-1], `${base.service==='ems'?'EMS':'Fire'} · ${width} × ${length} m area`);
    }
  }
  ...
  window.PTBO_SPAWN_BOX_EDITOR = Object.freeze({version:VERSION,sync:scheduleSync});
  start();
})();