/* Dispatch editor base-area + vehicle spawn editor — v1.6.47. */
(() => {
  'use strict';
  ...
  function setAreaCenter(lat,lng,moveSpawn = false,deltaLat = 0,deltaLng = 0) {
    if ($('b-lat')) $('b-lat').value = Number(lat).toFixed(6);
    if ($('b-lng')) $('b-lng').value = Number(lng).toFixed(6);
    if (moveSpawn && $('b-spawn-lat') && $('b-spawn-lng')) {
      $('b-spawn-lat').value = (numeric('b-spawn-lat',lat)+deltaLat).toFixed(6);
      $('b-spawn-lng').value = (numeric('b-spawn-lng',lng)+deltaLng).toFixed(6);
    }
  }
  ...
  function moveArea(event) {
    ...
    setAreaCenter(movingArea.lat+deltaLat,movingArea.lng+deltaLng,false);
