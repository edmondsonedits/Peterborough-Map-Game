/* Dispatch editor vehicle spawn box — v1.6.47. */
(() => {
  'use strict';
  ...
  function installStyle() {
    ...
    style.textContent = `
      ...
      .ptbo-spawn-box-icon{background:transparent!important;border:0!important;display:grid!important;place-items:center!important;overflow:visible!important}
      ...
      @media(max-width:760px){.ptbo-spawn-editor-fields{grid-template-columns:1fr 1fr;padding:10px}.ptbo-spawn-box-body{width:27px;height:48px}.ptbo-spawn-box-label{font-size:6px}}
    `;
  ...
  function installFields() {
    ...
    const existing = $('ptbo-spawn-editor-fields');
    ...
    section.innerHTML = `
      ...
      <p>Drag the vehicle box inside the station square. Its long side and arrow point in the direction the vehicle will face when it spawns.</p>
    ...
    form.addEventListener('submit', event => {
      ...
      if (!Number.isFinite(spawnLat) || !Number.isFinite(spawnLng) || !yard || !bases.contains(yard, spawnLat, spawnLng)) {
        ...
        setStatus('Move the vehicle spawn box inside the base square before saving.');
        return;
      }
      pendingSave = {
  ...
  function renderMarker(ensureValid = true) {
    ...
    if (!spawnMarker) {
      spawnMarker = L.marker(position,{draggable:true,icon:spawnIcon(heading),zIndexOffset:1600,keyboard:true,title:`${vehicleName} spawn position`}).addTo(editorMap);
      ...
      spawnMarker.on('dragend',event => {
        ...
        if (!shape || !bases.contains(shape,p.lat,p.lng)) {
          ...
          setStatus('The vehicle spawn box must stay inside the station square.');
          return;
        }
  ...
  function syncFromEditor() {
    installStyle();
    if (!$('ptbo-spawn-editor-fields') && !installFields()) return;
    if (!isBaseEditorOpen()) {
      loadedId = null;
      removeMarker();
      return;
  ...
  window.addEventListener('ptbo-bases-updated', () => {
    ...
    setStatus('Base and vehicle spawn saved on this device.');
  });
